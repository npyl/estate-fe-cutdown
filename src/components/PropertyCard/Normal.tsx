...
const PropertyCard = ({ item, selectedMarker }: PropertyCardProps) => {
  const { id, images, details, location, price, code, state, category, area } =
    item || {};
  ...
  const address = `${location?.region} ${location?.city} ${location?.complex}`;
    ...
    >
      ...
      <Stack px={2} py={2} spacing={0.8}>
        ...
        <SpaceBetween alignItems="center">
          <NormalBadge name={`${t("Code")}: ${code || ""}`} color="yellow" />
